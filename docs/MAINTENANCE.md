...
- Frontend HTML/JS dans `server/src/app/public/` (chargé via Express static).
- Authentification par JWT (payload: `id`, `role`, `company_id`).
- Migrations SQL dans `server/src/app/migrations/` appliquées au démarrage par `init-db.js`.
...
- Nouveau fichier: `server/src/app/migrations/XXX_description.sql` (numéro incrémental).
- Script d'initialisation `server/src/app/tools/init-db.js` exécute toutes les migrations triées par nom.
- Procédure:
...
2. Adapter payload JWT (`server/src/app/routes/auth/index.js`).
3. Adapter middleware d'accès (ex: filtrage entreprise dans `server/src/app/routes/lots/index.js`).
...
- Endpoints masquant correctement MOE pour `entreprise` (`server/src/app/routes/lots/index.js` GET `/:id`, `/:id/table`).
- Filtrage des offres par `company_id` pour rôle `entreprise` (implémenté).
- Filtrage des questions par `company_id` pour rôle `entreprise` (implémenté dans `server/src/app/routes/questions/index.js`).
...
- Fichier `server/src/app/routes/exports/index.js` gère Excel (lib `exceljs`).
